...
@interface RTCRoom() <TXLivePushListener, RoomMsgListener> {
    RoomMsgMgr              *_msgMgr;
    ...
    IEnterRoomCompletionHandler   _enterRoomCompletion;
}
@end
...
- (void)init:(NSString*)serverDomain accountInfo:(SelfAccountInfo *)userInfo withCompletion:(IInitCompletionHandler)completion {
    [self asyncRun:^{
        ...
        config.accountType = userInfo.accountType;
        config.userSig = userInfo.userSig;
        config.nickName = userInfo.nickName;
        config.headPicUrl = userInfo.headPicUrl;
...
- (NSMutableArray<PusherInfo *> *)parsePushersFromJsonArray:(NSArray *)pushers {
    ...
    for (id pusher in pushers) {
        ...
        pusherInfo.nickName = pusher[@"userName"];
        pusherInfo.headPicUrl = pusher[@"userAvatar"];
...
- (void)addRemoteView:(UIView *)view withUserID:(NSString *)userID {
    [self asyncRun:^{
        // 先检查房间是否存在userID
    ...
    [self asyncRun:^{
        ...
        if (EvtID == PUSH_EVT_PUSH_BEGIN) {
            if (_roomRole == 1) {  // 创建者
                ...
                NSDictionary *params = @{@"userID": _userInfo.userID, @"roomName": _roomName, @"userName": _userInfo.nickName,
                                         @"userAvatar": _userInfo.headPicUrl, @"pushURL": _pushUrl};
                ...
                NSDictionary *params = @{@"userID": _userInfo.userID, @"roomID": _roomID, @"userName": _userInfo.nickName,
                                         @"userAvatar": _userInfo.headPicUrl, @"pushURL": _pushUrl};
                ...
                [_httpSession POST:_apiAddr[kHttpServerAddr_AddPusher] parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
                    ...
                    [weakSelf asyncRun:^{
                        ...
                        if (errCode == 0) {
                            if (_enterRoomCompletion) {
                                _enterRoomCompletion(errCode, errMsg);
                                _enterRoomCompletion = nil;
                            }
                            
                        } else {
                            if (_enterRoomCompletion) {
                                _enterRoomCompletion(ROOM_ERR_ENTER_ROOM, [NSString stringWithFormat:@"%@[%d]", errMsg, errCode]);
                                _enterRoomCompletion = nil;
                            }
                        }
                    ...
                    [weakSelf asyncRun:^{
                        if (_enterRoomCompletion) {
                            _enterRoomCompletion(ROOM_ERR_REQUEST_TIMEOUT, @"网络请求超时，请检查网络设置");
                            _enterRoomCompletion = nil;
                        }
                    }];
...
- (void)onRecvGroupTextMsg:(NSString *)groupID userID:(NSString *)userID textMsg:(NSString *)textMsg nickName:(NSString *)nickName headPic:(NSString *)headPic {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (_delegate && [_delegate respondsToSelector:@selector(onRecvRoomTextMsg:userID:nickName:headPic:textMsg:)]) {
            [_delegate onRecvRoomTextMsg:groupID userID:userID nickName:nickName headPic:headPic textMsg:textMsg];
        }
    });
}
    ...
    [self getPusherList:^(int errCode, NSString *errMsg, RoomInfo *roomInfo) {
        ...
        for (PusherInfo *pusherInfo in newRoomInfo.pusherInfoArray) {
            ...
            if (isNewMember) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    [self sendDebugMsg:[NSString stringWithFormat:@"加入房间: userID[%@] nickName[%@] playUrl[%@]", pusherInfo.userID, pusherInfo.nickName, pusherInfo.playUrl]];
                });
            } else {
        ...
        for (PusherInfo *pusherInfo in leaveSet) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [self sendDebugMsg:[NSString stringWithFormat:@"离开房间: userID[%@] nickName[%@]", pusherInfo.userID, pusherInfo.nickName]];
            });
        }
