...
- (void)getRoomSig:(IGetRoomSigCompletion)completion {
    [self loginAppServer:_userID pwd:_pwd sdkappid:_sdkappid withCompletion:^(NSString *userSig) {
        if (userSig) {
            [self requestSigServer:_userID userSig:userSig roomID:_roomID sdkappid:_sdkappid withCompletion:^(NSString *roomSig) {
                if (completion) {
                    completion(roomSig);
                }
            }];
