...
- (void) setCacheStrategy:(NSInteger) nCacheStrategy
{
    ...
    switch (_cacheStrategy) {
        case CACHE_STRATEGY_FAST:
            ...
            _config.cacheTime = CACHE_TIME_FAST;
...
- (void)clickNewUrl:(UIButton*)btn
{
    ...
    if (_isRealtime) {
        [TCHttpUtil asyncSendHttpRequest:@"get_test_rtmpaccurl" handler:^(int result, NSDictionary *resultDict) {
            if (result != 0) {
